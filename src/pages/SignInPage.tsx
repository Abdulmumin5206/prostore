...
const SignInPage: React.FC = () => {
  ...
  const { signIn, enableAdminOverride, isSupabaseConfigured } = useAuth()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Dev admin override: allow admin/admin or admin@example.com/admin
    const isAdminOverride =
      (trimmedEmail.toLowerCase() === 'admin' || trimmedEmail.toLowerCase() === 'admin@example.com') &&
      password === 'admin'
    ...
    if (!isSupabaseConfigured) {
      setError('Supabase is not configured. Use admin/admin to access the Admin dashboard for development.')
      setLoading(false)
      return
    }
      ...
      <div className="max-w-md w-full space-y-8">
        <div className="flex flex-col items-center">
          ...
          <p className="mt-2 text-center text-xs text-gray-500 dark:text-gray-400">
            Dev admin override: <span className="font-mono">admin</span> or <span className="font-mono">admin@example.com</span> / <span className="font-mono">admin</span>
          </p>
          <p className="mt-2 text-center text-sm text-gray-600 dark:text-gray-400">
            Or{' '}
            <Link
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <div>
            ...
            >
              {cooldownSeconds > 0 ? `Wait ${cooldownSeconds}s` : loading ? 'Signing in...' : 'Sign In'}
            </button>
          </div>
        </form>
      </div>
